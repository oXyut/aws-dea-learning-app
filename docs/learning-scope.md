...
複数選択では、正しい選択肢の選び漏れや誤った選択肢の追加がある場合は不正解です。確定済み回答は同じブラウザのlocalStorageへ保存し、再読み込み後も復元します。

2026-09-21追記：この表は応用20テーマの対応表です。アプリには別途、23章の本文・24問の章末確認・30項目の用語辞典を追加しました。全17タスクへの対応と未収録範囲は画面の「この教材の範囲と到達基準」、評価は[敵対的レビュー](adversarial-review-2026-09-21.md)を参照してください。
